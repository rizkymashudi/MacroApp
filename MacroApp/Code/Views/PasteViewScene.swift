...
struct PasteViewScene: View {
    ...
    @State var rawTexts : String = ""
//    @ObservedObject var apiBackend = ApiServiceBackend()
        ...
        NavigationView{
            ZStack{
                ...
                VStack(alignment: .center, spacing: 10){
                    NavigationLink(destination: YudistiraListView(yourText: $yourText), isActive: $isShowingDetailView){ }
                    .hidden()
                    Spacer(minLength: 30)
                    ...
                    CopastView(yourText: $yourText,rawTexts: $rawTexts)
                    Spacer(minLength: 20)
                    
                    Button(action: {
                        print("Button Tapped")
//                        apiBackend.getText(rawTexts: rawTexts)
                        apiYudistira.getText(rawTexts: rawTexts)
